<?php

// Namespace
namespace BMI\Plugin\External;

// Use
use BMI\Plugin\Backup_Migration_Plugin as BMP;
use BMI\Plugin\BMI_Logger as Logger;
use BMI\Plugin\BMI_Pro_Core;
use BMI\Plugin\BMProAjax as BMProAjax;
use BMI\Plugin\Scanner\BMI_BackupsScanner as Backups;
use BMI\Plugin\Dashboard as Dashboard;

// Exit on direct access
if (!defined('ABSPATH')) {
  exit;
}

/**
 * BMI_External_onedrive
 */
class BMI_External_OneDrive
{

  private $onedrive_secret = false;

  public function __construct()
  {

    add_action('bmi_premium_remove_backup_file', [&$this, 'deleteBackup']);
    add_action('bmi_premium_remove_backup_json_file', [&$this, 'deleteBackupManifest']);
  }

  /**
   * checkForBackupsToUpload - Will check for backups that requires to be in sync with cloud
   *
   * @return string[]
   */
  public function checkForBackupsToUpload()
  {
    $isEnabled = Dashboard\bmi_get_config('STORAGE::EXTERNAL::ONEDRIVE');
    if (!($isEnabled === true || $isEnabled === 'true')) {
      update_option('bmip_to_be_uploaded', ['current_upload' => [], 'queue' => []]);
      return;
    }

    // Upload Object
    $requiresUpload = get_option('bmip_to_be_uploaded', [
      'current_upload' => [],
      'queue' => [],
      'failed' => []
    ]);

    // Local Backups
    require_once BMI_INCLUDES . DIRECTORY_SEPARATOR . 'scanner' . DIRECTORY_SEPARATOR . 'backups.php';
    $backups = new Backups();
    $backupsAvailable = $backups->getAvailableBackups("local");
    $localBackups = $backupsAvailable['local'];
    $localBackups = array_reverse($localBackups);
    $uploadedBackupStatus = get_option('bmi_uploaded_backups_status', []);

    // OneDrive
    $files = $this->parseFiles($this->getAllFiles());

    if (!$files) return;

    foreach ($localBackups as $name => $details) {

      $md5 = $details[7];
      if (isset($uploadedBackupStatus[$md5]) && isset($uploadedBackupStatus[$md5]['onedrive'])) {
          continue;
      }
      if (!(isset($files['manifests'][$md5 . '.json']) && isset($files['backups'][$name]))) {

        $task = 'onedrive_' . $md5;

        // File is not uploaded action required
        if (!isset($requiresUpload['queue'][$task])) {
          $isAnyTaskATM = isset($requiresUpload['current_upload']['task']);


          // if (isset($requiresUpload['failed'][$task])) unset($requiresUpload['failed'][$task]);

          if (($isAnyTaskATM && $requiresUpload['current_upload']['task'] != $task) || !$isAnyTaskATM) {
            $requiresUpload['queue'][$task] = [
              'name' => $name,
              'md5' => $md5,
              'json' => $md5 . '.json'
            ];
          }
        }
      }
    }

    update_option('bmip_to_be_uploaded', $requiresUpload);
    return ['status' => 'success'];
  }

  public function parseFiles($files)
  {

    $parsedFiles = ['backups' => [], 'manifests' => []];

    if ($files === false) return false;

    foreach ($files as $index => $file) {
      if (isset($file['folder'])) {
        continue; //Skip directories
      }

      $extension = pathinfo($file['name'], PATHINFO_EXTENSION);

      if (BMI_PRO_DEBUG) {
        // Logger::error("parseFiles - " . $file['name'] . " - " . print_r($file['file'], true));
        // Logger::error("parseFiles - ext - $extension");
      }

      if (in_array($extension, ["zip", "tar", "gz"])) {
        $type = 'backups';
      }

      if ($extension === 'json') {
        $type = 'manifests';
      }

      if (isset($type)) {
        $parsedFiles[$type][$file['name']] = [
          'size' => $file['size']
        ];
        unset($type);
      }
    }


    if (BMI_PRO_DEBUG) {
      // Logger::error("parseFiles - " . print_r($parsedFiles, true));
    }

    return $parsedFiles;
  }

  function _check_token_validity($access_token) {
    // Microsoft Graph API endpoint to check OneDrive access
    $url = 'https://graph.microsoft.com/v1.0/me/drive';

    // Prepare headers, including the access token
    $headers = array(
        'Authorization' => 'Bearer ' . $access_token,
        'Accept'        => 'application/json',
    );

    $response = wp_remote_get($url, array(
        'headers' => $headers,
    ));

    
    if (is_wp_error($response)) {
        return false; // Error in sending request
    }

    
    $response_code = wp_remote_retrieve_response_code($response);

    
    if ($response_code === 200) {
        return true; // Token is valid
    } else {
        return false; // Token is invalid or expired
    }
}


  public function getSecret()
  {
    $tempKeyOneDriveFile = BMI_TMP . DIRECTORY_SEPARATOR . 'onedriveKeys.php';
    if (file_exists($tempKeyOneDriveFile)) {
      $onedriveKeys = file_get_contents($tempKeyOneDriveFile);
      if (strpos($onedriveKeys, "\n") !== false) {
        $lines = explode("\n", $onedriveKeys);
        if (sizeof($lines) == 3) {
          $onedrive_wid = substr($lines[1], 2);
          if (function_exists('wp_load_alloptions')) {
            wp_load_alloptions(true);
          }
          delete_option("bmi_pro_onedrive_wid");
          if (function_exists('wp_load_alloptions')) {
            wp_load_alloptions(true);
          }
          update_option("bmi_pro_onedrive_wid", $onedrive_wid);
        }
      }
      if (strpos(site_url(), 'tastewp') !== false) {
        if (function_exists('wp_load_alloptions')) {
          wp_load_alloptions(true);
        }

        update_option('__tastewp_redirection_performed', true);
        update_option('auto_smart_tastewp_redirect_performed', 1);
        update_option('tastewp_auto_activated', true);
        update_option('__tastewp_sub_requested', true);
      }

      unlink($tempKeyOneDriveFile);
    }

    $uri = home_url();
    if (substr($uri, 0, 4) != 'http') {
      if (is_ssl()) $uri = 'https://' . home_url();
      else $uri = 'http://' . home_url();
    }

    $wid = get_option('bmi_pro_onedrive_wid', false);

    if ($wid === false) {
      return false;
    }

    $savedSecret = get_transient('bmi_pro_onedrive_secret');
    if ($savedSecret) {
      // if (!$this->_check_token_validity($savedSecret))
      // {
      //   delete_transient('bmi_pro_onedrive_secret');
      // }
      // else
      // {
        $this->removeNotice("invalid_wid");
        $this->removeNotice("invalid_token");
        return $savedSecret;
      // }
    }

    $url = 'https://authentication.backupbliss.com/oauth/onedrive/verify';
    $response = wp_remote_post($url, array(
      'method' => 'POST',
      'timeout' => 15,
      'redirection' => 2,
      'httpversion' => '1.0',
      'blocking' => true,
      'body' => array(
        'wid' => $wid,
        'site' => $uri
      )
    ));

    if (is_wp_error($response)) {
      $error_message = $response->get_error_message();
      Logger::error('[BMI PRO] Something went wrong during OneDrive getting token:' . $error_message);
      return false;
    } else {

      $http_code = wp_remote_retrieve_response_code($response);
      if (BMI_PRO_DEBUG) {
        Logger::error("[BMI PRO] getSecret: $http_code - " . print_r($response['body'], true));
      }
      if ($http_code == 200) {
        $this->removeNotice("invalid_wid");
        $result = json_decode($response['body']);


        if (isset($result->wid) && $wid === $result->wid) {
          if (isset($result->token_expiration) && isset($result->token)) {
            
            if (!$this->_check_token_validity($result->token)) {
              $message = 'There was an error authenticating your OneDrive account.<br />';
              $message .= 'Please click <a href="javascript:document.getElementById(\'onedrive-connect-btn\').click();">here</a> to re-authenticate.';
              $message .= ' Or click <a href="javascript:document.getElementById(\'bmi-pro-storage-onedrive-toggle\').checked=false;document.querySelector(\'#storage-options .save-btn\').click(); setTimeout(()=>{window.location.reload()}, 500);">here</a> to disable OneDrive external storage.';

              $this->showNotice("invalid_token", $message);
              return false;
            }
            $expiresInSeconds = intval($result->token_expiration) - intval(microtime(true));
            $accessToken = $result->token;
            set_transient('bmi_pro_onedrive_secret', $accessToken, $expiresInSeconds);
            $this->onedrive_secret = $accessToken;
            return $this->onedrive_secret;
          }
        }
      } else if ($http_code == 404) {
        Logger::error("[BMI PRO] This site's OneDrive WID ($wid) was not found in the authentication server. User must re-authenticate.");
        $message = 'There was an error while retrieving OneDrive credentials from the authentication server.<br />';
        $message .= 'Please click <a href="javascript:document.getElementById(\'onedrive-connect-btn\').click();">here</a> to re-authenticate.';
        $message .= ' Or click <a href="javascript:document.getElementById(\'bmi-pro-storage-onedrive-toggle\').checked=false;document.querySelector(\'#storage-options .save-btn\').click(); setTimeout(()=>{window.location.reload()}, 500);">here</a> to disable OneDrive external storage.';

        $this->showNotice("invalid_wid", $message);
      }
      return false;
    }
  }

  public function showNotice($type, $message, $time = 0)
  {
    if (BMI_PRO_DEBUG) {
      Logger::error("showNotice($type, $message, $time)");
    }

    set_transient('bmip_onedrive_notice_' . $type, $message, $time);
    $transients = [];
    $current_trasients = get_transient('bmip_onedrive_notices');
    if ($current_trasients) $transients = $current_trasients;
    $transients[$type] = $type;
    set_transient('bmip_onedrive_notices', $transients);
  }

  public function removeNotice($type)
  {
    // if (BMI_PRO_DEBUG) {
    //   Logger::error("removeNotice($type)");
    // }

    delete_transient('bmip_onedrive_notice_' . $type);
    $current_trasients = get_transient('bmip_onedrive_notices');
    if (isset($current_trasients[$type])) {
      unset($current_trasients[$type]);
      set_transient('bmip_onedrive_notices', $current_trasients);
    }
  }

  public function getNotice($type)
  {
    // if (BMI_PRO_DEBUG) {
    //   Logger::error("getNotice($type)");
    // }

    return get_transient("bmip_onedrive_notice_" . $type);
  }

  public function getNotices()
  {
    $bmip_onedrive_notices = get_transient("bmip_onedrive_notices");
    $temp_notices = $bmip_onedrive_notices;
    $notices = [];
    if ($bmip_onedrive_notices) {
      foreach ($bmip_onedrive_notices as $notice) {
        $noticemessage = get_transient("bmip_onedrive_notice_" . $notice);
        if ($noticemessage) {
          $notices[] = $noticemessage;
        } else {
          unset($temp_notices[$notice]);
        }
      }
    }

    if ($bmip_onedrive_notices !== $temp_notices) {
      set_transient("bmip_onedrive_notices", $temp_notices);
    }

    return $notices;
  }


  private function _makeApiCall($url, $req_type = "GET", $body = [], $custom_headers = null)
  {
    if (BMI_PRO_DEBUG) {
      $backtrace = debug_backtrace();
      // Get the caller's function name
      $callerFunction = isset($backtrace[1]['function']) ? $backtrace[1]['function'] : 'unknown';
    }
    if (BMI_PRO_DEBUG) {
      // Logger::error("[BMI PRO] REQUEST FROM $callerFunction() in _makeApiCall($url, $req_type, " . ($req_type != "PUT" ? print_r($body, true) : "BINARYDATA") . ", " . print_r($custom_headers, true) . ")");
    }

    $secret = $this->getSecret();
    if (!$secret) {
      return ["status" => false];
    }

    $max_execution_time_pre_limit = ini_get('max_execution_time') - 2;

    $headers = $custom_headers == null ? [
      'Authorization: Bearer ' . $secret,
      'Accept: application/json',
      'Content-Type: application/json',
    ] : $custom_headers;



    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, $req_type);
    curl_setopt($ch, CURLOPT_TIMEOUT, $max_execution_time_pre_limit);

    if ($req_type == "POST") {
      curl_setopt($ch, CURLOPT_POST, true);
      curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($body));
    } elseif ($req_type == "PUT") {
      curl_setopt($ch, CURLOPT_POSTFIELDS, $body);
    }

    $response = curl_exec($ch);
    $http_code = curl_getinfo($ch, CURLINFO_HTTP_CODE);

    if ($req_type == "DELETE") {
      return $http_code;
    }

    if (curl_errno($ch)) {
      Logger::error("[BMI PRO] Error in _makeApiCall request: Type: $req_type HTTP Code: $http_code. Error: " . curl_error($ch));
      return ["status" => false, "http_code" => $http_code, "response_data" => $response];
    }

    if (is_resource($ch)) {
      curl_close($ch);
    }

    if ($http_code >= 200 && $http_code <= 299) {
      $response_data = $custom_headers == null ? json_decode($response, true) : $response;
      if (BMI_PRO_DEBUG) {
        //Logger::error("[BMI PRO] RESPONSE _makeApiCall - " . print_r($response_data, true));
      }
      return ["status" => true, "http_code" => $http_code, "response_data" => $response_data];
    } else {
      if ($http_code == 401 || $http_code == 403) {
        //Check if fragment upload issue
        $error_message = json_decode($response);
        if (!$error_message) {
          if (isset($error_message->error->code)) {
            if ($error_message->error->code == "invalidRange") {
              $invalidRange = true;
            }
          }
        }
        if (!isset($invalidRange) || $http_code == 403) {
          //Most likely the token has expired, so force renew it in the next request by deleting the transient
          delete_transient("bmi_pro_onedrive_secret");
        }
      }
      Logger::error("[BMI PRO] Error in _makeApiCall request: Type: $req_type HTTP Code: $http_code. Response: $response");
      return ["status" => false, "http_code" => $http_code, "response_data" => $response];
    }
  }

  public function getFileDetailByName($file_name)
  {
    if (BMI_PRO_DEBUG) {
      Logger::error("getFileDetailByName $file_name");
    }
    // URL to search for the file by name within the AppFolder
    // $url = "https://graph.microsoft.com/v1.0/me/drive/special/approot/search(q='{$file_name}')";
    // Get direct file by name
    $url = "https://graph.microsoft.com/v1.0/me/drive/special/approot:/{$file_name}";

    $response_data = $this->_makeApiCall($url);


    // Check if any files matched the search
    if ($response_data["status"]) {
      $response_data = $response_data["response_data"];
      if ($response_data) {
        //if (isset($response_data['value']) && count($response_data['value']) > 0) {
        if (BMI_PRO_DEBUG) {
          //Logger::error("getFileDetailByName - File found - " . print_r($response_data['value'][0], true));
          Logger::error("getFileDetailByName - File found - " . print_r($response_data, true));
        }
        // Return the first matching file's details (or handle as needed)
        // return $response_data['value'][0];
        return $response_data;
      } else {
        Logger::error("[BMI PRO] (getFileDetailByName) Requested file $file_name was not found");
        return false;
      }
    }

    return false; // File not found
  }

  private function deleteFile($file_path)
  {
    if (BMI_PRO_DEBUG) {
      Logger::error("deleteFile($file_path)");
    }
    // Construct the URL for the file deletion, assuming the file_path is relative to the root
    $url = "https://graph.microsoft.com/v1.0/me/drive/special/approot:/{$file_path}";

    $http_code = $this->_makeApiCall($url, "DELETE");

    // Handle the response based on the status code
    if ($http_code == 204) {
      // 'File deleted successfully'
      return True;
    } else {
      //Logger::error("[BMI PRO] Error in deleteFile. HTTP Code: $http_code.");
      return False;
    }
  }

  public function deleteBackup($md5)
  {

    if (BMI_PRO_DEBUG) {
      Logger::error("deleteBackup($md5)");
    }

    $manifest = $this->getManifest($md5);
    if ($manifest) {
      $this->deleteFile($manifest->name);
    }
  }

  public function getManifest($md5)
  {
    $manifest = false;
    $localManifest = BMI_BACKUPS . DIRECTORY_SEPARATOR . $md5 . '.json';

    if (file_exists($localManifest)) {

      $manifestData = file_get_contents($localManifest);
      $manifest = json_decode($manifestData);
    } else {

      $manifestData = $this->getFile($md5 . '.json');
      if (is_array($manifestData) && $manifestData["file_data"]) {

        $manifest = json_decode($manifestData["file_data"]);
      }
    }
    return $manifest;
  }

  public function deleteBackupManifest($md5_json)
  {
    if (BMI_PRO_DEBUG) {
      Logger::error("deleteBackupManifest($md5_json)");
    }

    $this->deleteFile($md5_json);
  }

  public function initiateUploadSession($file_path)
  {

    $file_name = basename($file_path);

    if (BMI_PRO_DEBUG) {
      Logger::error("[BMI PRO] initiateUploadSession - $file_path - $file_name");
    }


    $url = 'https://graph.microsoft.com/v1.0/drive/special/approot:/' . $file_name . ':/createUploadSession';
    $body = [
      'item' => [
        '@microsoft.graph.conflictBehavior' => 'replace',
        'name' => $file_name
      ]
    ];

    $response = $this->_makeApiCall($url, "POST", $body);
    if ($response["status"]) {
      $session = $response["response_data"];
      if (!empty($session['uploadUrl'])) {
        $this->removeNotice("upload_issue_space");
        delete_option("bmip_onedrive_required_space");
        return $session;
      } else {
        Logger::error("[BMI PRO] Failed to create upload session: " . json_encode($session));
        return false;
      }
    } else {
      $code = intval($response['http_code']);
      if ($code == 507) {
     
        $error_message_notice = 'Upload to OneDrive could not finish, due to insufficient storage.<br />';
        if (is_string($response['response_data'])) {
          $error = json_decode($response['response_data']);
          if (isset($error->error->message)) {
            $errorMessage = $error->error->message;
            $error_message_notice .= 'Received message: <i>' . $errorMessage . '</i><br />';
          }
        }
        $error_message_notice .= 'Plugin will start uploading as soon as there\'s enough free space.<br />';
        $error_message_notice .= 'In the meantime, please try to clear some space on your OneDrive.';
  
        
        add_option("bmip_onedrive_required_space", filesize($file_path));
        $this->showNotice("upload_issue_space", $error_message_notice, 60 * 60);
      } elseif ($code == 429) {
       
        $error_message_notice = 'Upload to OneDrive could not finish, due to rate limit error.<br />';
        $error_message_notice .= 'Received message: <i>Too Many Requests in a short amount of time.</i><br />';
        $error_message_notice .= 'Plugin will retry uploading automatically after 2 minutes.<br />';
  
        $this->showNotice('upload_issue', $error_message_notice, 60 * 2);
      }
    }
  }

  private function uploadChunkWithSession($upload_session, $chunk_data, $start_byte, $end_byte, $total_size)
  {
    $headers = [
      'Content-Range: bytes ' . $start_byte . '-' . $end_byte . '/' . $total_size,
      'Content-Length: ' . strlen($chunk_data)
    ];

    if (BMI_PRO_DEBUG) {
      // Logger::error("[BMI PRO] BEFORE UPLOAD uploadChunkWithSession(" . $upload_session['uploadUrl'] . ", $start_byte, $end_byte, $total_size" . ")\n" . print_r($headers, true));
    }
    $response = $this->_makeApiCall($upload_session['uploadUrl'], "PUT", $chunk_data, $headers);

    if ($response["status"]) {
      if (BMI_PRO_DEBUG) {
        // Logger::error("[BMI PRO] AFTER UPLOAD uploadChunkWithSession\n" . print_r($response, true));
      }
      return $response;
    }

    Logger::error("[BMI PRO] Failed to upload chunks. Start byte: $start_byte. End byte: $end_byte. Total Size: $total_size");
    return $response;
  }

  public function getAllFiles()
  {
    $url = 'https://graph.microsoft.com/v1.0/me/drive/special/approot/children';
    $response = $this->_makeApiCall($url);
    if (BMI_PRO_DEBUG) {
      // Logger::error("[BMI PRO] getAllBackups " . print_r($response, true));
    }
    if ($response["status"]) {
      $files = $response["response_data"];
      return $files['value'];
    }

    return false;
  }

  private function downloadFile($file_details, $start_byte = 0, $end_byte = null)
  {
    if (BMI_PRO_DEBUG) {
      //Logger::error("downloadFile(" . print_r($file_details, true) . ", $start_byte, $end_byte)");
    }
    if (!isset($file_details['@microsoft.graph.downloadUrl'])) {
      Logger::error('[BMI PRO] Download URL not available in the file details.');
      return false;
    }

    $headers = [];

    // Set the Range header for partial download
    if ($end_byte !== null) {
      $headers = [
        'Range: bytes=' . $start_byte . '-' . $end_byte
      ];
    } else {
      $headers = [
        'Range: bytes=' . $start_byte . '-'
      ];
    }

    // Get the download URL from the file details
    $download_url = $file_details['@microsoft.graph.downloadUrl'];

    $response = $this->_makeApiCall($download_url, "GET", [], $headers);

    if ($response["status"]) {
      return $response["response_data"];
    }

    return false;
  }

  public function getFile($file_name, $start_byte = 0, $end_byte = null)
  {
    if (BMI_PRO_DEBUG) {
      Logger::error("getFile $file_name");
    }
    $file_detail = $this->getFileDetailByName($file_name);
    if ($file_detail) {
      return ["file_detail" => $file_detail, "file_data" => $this->downloadFile($file_detail, $start_byte, $end_byte)];
    }
    return false;
  }

  public function getStorageInfo()
  {
    $url = "https://graph.microsoft.com/v1.0/me/drive";

    $response = $this->_makeApiCall($url);

    if (BMI_PRO_DEBUG) {
      Logger::error("getStorageInfo - " . print_r($response, true));
    }

    if ($response["status"]) {
      return $response["response_data"];
    }

    return [];
  }

  public function uploadFile($uploadSession, $filePath, $manifestPath, $md5, $batch, $bytesPerRequest)
  {

    if (!file_exists($filePath)) {

      update_option('bmip_to_be_uploaded', [
        'current_upload' => [],
        'queue' => [],
        'failed' => []
      ]);

      return ['status' => 'error'];
    }

    set_transient('bmip_upload_ongoing', '1', 31);

    $batchNumber = intval($batch);
    $maxLength = filesize($filePath);

    # Microsoft recommended multiple value 320Kb
    $chunkSize = 4 * 327680 * intval($bytesPerRequest / 1024 / 1024);

    # Limit the chunk size to max of 50MB with multiple of recommended value
    $maxChunkSize = 50 * 1024 * 1024;
    $chunkSize = $chunkSize > $maxChunkSize ? $maxChunkSize : $chunkSize;

    $chunkOffset = (($batchNumber - 1) * $chunkSize);
    $rangeEnd = (($chunkSize * $batchNumber) - 1);

    if ($rangeEnd >= $maxLength) $rangeEnd = $maxLength - 1;
    if (($chunkSize + $chunkOffset) >= $maxLength) $chunkSize = $rangeEnd - $chunkOffset + 1;
    $nextShouldStartAt = $rangeEnd + 1;


    if ($stream = fopen($filePath, 'r')) {
      $binaryData = stream_get_contents($stream, $chunkSize, $chunkOffset);
      fclose($stream);
    }


    $toBeUploaded = get_option('bmip_to_be_uploaded', false);

    if (BMI_PRO_DEBUG)
      Logger::error("Before uploadFile - " . print_r($toBeUploaded['current_upload']['batch'], true));

    $response = $this->uploadChunkWithSession($uploadSession, $binaryData, $chunkOffset, $rangeEnd, $maxLength);


    $code = intval($response['http_code']);


    // if (BMI_PRO_DEBUG){
    //   $code = 507;
    //   $response["response_data"] = null;
    //   $response["status"] = false;
    // }

    if ($response["status"]) {
     
      //Chunk accepted, let's continue uploading
      if ($code == 202) {
       
        $task = $toBeUploaded['current_upload']['task'];
        if (!isset($toBeUploaded['failed'])) $toBeUploaded['failed'] = [];
        if (isset($toBeUploaded['failed'][$task])) unset($toBeUploaded['failed'][$task]);

        $toBeUploaded['current_upload']['batch'] = intval($batch) + 1;
        $toBeUploaded['current_upload']['progress'] = number_format(($rangeEnd / $maxLength) * 100, 2) . '%';
        update_option('bmip_to_be_uploaded', $toBeUploaded);

        if (BMI_PRO_DEBUG)
          Logger::error("After uploadFile - " . print_r($toBeUploaded['current_upload']['batch'], true));

        $test = get_option('bmip_to_be_uploaded', false);

        if (BMI_PRO_DEBUG)
          Logger::error("After updating option uploadFile - " . print_r($test['current_upload']['batch'], true));
      }
      // Upload finished, upload manifest now
      elseif ($code == 200 || $code == 201) {

        $manifestUploadSession = $this->initiateUploadSession($manifestPath);
        if ($stream = fopen($manifestPath, 'r')) {
          $binaryData = stream_get_contents($stream);
          fclose($stream);
        }

        $size = strlen($binaryData);
        $manifestRes = $this->uploadChunkWithSession($manifestUploadSession, $binaryData, 0, $size - 1, $size);
        if ($manifestRes['status'] == true) {
          $uploadedBackupStatus = get_option('bmi_uploaded_backups_status', []);
          if (!isset($uploadedBackupStatus[$md5])) {
            $uploadedBackupStatus[$md5] = [];
          }
          $uploadedBackupStatus[$md5]['onedrive'] = true;
          update_option('bmi_uploaded_backups_status', $uploadedBackupStatus);
        }

        $task = $toBeUploaded['current_upload']['task'];
        $toBeUploaded['current_upload'] = [];
        if (!isset($toBeUploaded['failed'])) $toBeUploaded['failed'] = [];
        if (isset($toBeUploaded['failed'][$task])) unset($toBeUploaded['failed'][$task]);

        $this->removeNotice("upload_issue_space");
        delete_option("bmip_onedrive_required_space");

        update_option('bmip_to_be_uploaded', $toBeUploaded);
        
        do_action('bmi_backup_upload_completed', $md5);
      }
    } elseif ($code == 507) {
     
      $error_message_notice = 'Upload to OneDrive could not finish, due to insufficient storage.<br />';
      if (is_string($response['response_data'])) {
        $error = json_decode($response['response_data']);
        if (isset($error->error->message)) {
          $errorMessage = $error->error->message;
          $error_message_notice .= 'Received message: <i>' . $errorMessage . '</i><br />';
        }
      }
      $error_message_notice .= 'Plugin will start uploading as soon as there\'s enough free space.<br />';
      $error_message_notice .= 'In the meantime, please try to clear some space on your OneDrive.';

      
      add_option("bmip_onedrive_required_space", filesize($filePath));
      $this->showNotice("upload_issue_space", $error_message_notice, 60 * 60);
    } elseif ($code == 429) {
     
      $error_message_notice = 'Upload to OneDrive could not finish, due to rate limit error.<br />';
      $error_message_notice .= 'Received message: <i>Too Many Requests in a short amount of time.</i><br />';
      $error_message_notice .= 'Plugin will retry uploading automatically after 2 minutes.<br />';

      $this->showNotice('upload_issue', $error_message_notice, 60 * 2);
    } else {

      Logger::error('[BMI PRO] Error during file upload (OneDrive) code:' . $code);
      if (isset($response['response_data']) && is_string($response['response_data'])) {
        Logger::error('[BMI PRO] Message received (body):' . print_r($response['response_data'], true));
      }

      $error_message_notice = 'Upload to OneDrive could not finish, due to an error.<br />';
      if (is_string($response['response_data'])) {
        $error = json_decode($response['response_data']);
        if (isset($error->error->message)) {
          $errorMessage = $error->error->message;
          $error_message_notice .= "Code: $code - Received message:<i>" . $errorMessage . '</i><br />';
        } else if ($error == null) {
          $error_message_notice .= "HTTP Error Response - " . $response["response_data"];
        }
      }

      if ($code == 0) {
        $error_message_notice .= "Received Message: <i>Connection timed out. (This is most likely due to a connection issue in your server.)</i><br />";
      }



      $error_message_notice .= "Plugin will retry uploading automatically within a minute since this error.";
      $this->showNotice("upload_issue", $error_message_notice, 60);
    }


    if (isset($error_message_notice)) {
      $task = $toBeUploaded['current_upload']['task'];
      // Requeueing is handled globally
      // $toBeUploaded['queue'][$task] = [
      //   'name' => $toBeUploaded['current_upload']['name'],
      //   'md5' => $toBeUploaded['current_upload']['md5'],
      //   'json' => $toBeUploaded['current_upload']['json']
      // ];

      $toBeUploaded['current_upload'] = [];
      if (!isset($toBeUploaded['failed'])) $toBeUploaded['failed'] = [];
      if (isset($toBeUploaded['failed'][$task])) $toBeUploaded['failed'][$task]++;
      else $toBeUploaded['failed'][$task] = 1;

      update_option('bmip_to_be_uploaded', $toBeUploaded);
    }

    delete_transient('bmip_upload_ongoing');
    return ['status' => 'success', 'data' => $response];
  }

  public function verifyConnection()
  {

    $baseurl = home_url();
    if (substr($baseurl, 0, 4) != 'http') {
      if (is_ssl())
        $baseurl = 'https://' . home_url();
      else
        $baseurl = 'http://' . home_url();
    }

    $wid = get_option('bmi_pro_onedrive_wid', false);

    if ($wid === false) {
      return ['status' => 'success', 'result' => 'disconnected'];
    }

    if (get_transient("bmi_pro_onedrive_secret")) {
      return ['status' => 'success', 'result' => 'connected'];
    }

    $url = 'https://authentication.backupbliss.com/oauth/onedrive/verify';
    $response = wp_remote_post($url, array(
      'method' => 'POST',
      'timeout' => 15,
      'redirection' => 2,
      'httpversion' => '1.0',
      'blocking' => true,
      'body' => array(
        'wid' => $wid,
        'site' => $baseurl
      )
    ));

    $res = 'disconnected';
    if (is_wp_error($response)) {
      $error_message = $response->get_error_message();
      Logger::error('[BMI PRO] Something went wrong during OneDrive connection verification:' . $error_message);
      return ['status' => 'error', 'result' => 'disconnected'];
    } else {
      $result = json_decode($response['body']);

      if (isset($result->wid) && $wid === $result->wid) {
        if (isset($result->token_expiration) && isset($result->token)) {
          $expiresInSeconds = intval($result->token_expiration) - intval(microtime(true));
          $accessToken = $result->token;
          set_transient('bmi_pro_onedrive_secret', $accessToken, $expiresInSeconds);
          $res = 'connected';
        }
      }
      return ['status' => 'success', 'result' => $res];
    }
  }
}
