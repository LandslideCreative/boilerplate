...
final class ZipFormat
{
    ...
    public static function buildLocalFileHeaderWithDataDescriptor(
        ...
        $bitFlags = self::getGeneralPurposeBitFlag($filename, true);

        // Version needed
        $versionNeeded = $useZip64
            ? self::VERSION_NEEDED_ZIP64
            : self::getVersionNeeded($compressionMethod, false);

        // Build extra field for ZIP64 if needed (with placeholder zeros)
        $extraField = '';
        if ($useZip64) {
        ...
        $header = pack(
            ...
            $compressionMethod,              // 2 bytes: compression method
